...
QUnit.test("isosceles triangle has two equal sides", function( assert ) {
	...
	assert.equal(triangle.isIsosceles(1,1,7),true,"1,1,7 is isosceles") ;
	assert.equal(triangle.isIsosceles(3,2,5),true,"3,2,5 is isosceles") ;
	assert.equal(triangle.isIsosceles(0,0,1),true,"0,0,1 is isosceles") ;
	assert.equal(triangle.isIsosceles(-1,-1,5),true,"-1,-1,5 is isosceles") ;
});
QUnit.test("scalene triangle has three different sides", function( assert ) {
	...
	assert.equal(triangle.isScalene(1,2,6),true,"1,2,6 is scalene") ;
});
